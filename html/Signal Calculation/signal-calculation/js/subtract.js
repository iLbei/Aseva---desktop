...
$("button").click(function () {
  ...
  changeVal();
  var text = node1 + "-" + node2;
  $("p").text(text).attr("title", text);
})
...
function changeVal() {
  ...
  if (edit == "false") {
    if (!flag) {
      ...
      var box = new Box(90, 30, name, "Subtract signal", selected_node_id, "orange", rank, 0, 0, parentId, [], true, 'sub_signal');
      boxArr.push(box);
    } else {
      if (swap)[parentId[0], parentId[1]] = [parentId[1], parentId[0]];
      name = "{" + parentId[0] + "}-" + "{" + parentId[1] + "}";
      boxArr[boxArr.length - 1] = new Box(90, 30, name, "Subtract signal", selected_node_id, "orange", rank, 0, 0, parentId, [], true, 'sub_signal');
    }
  } else {
    ...
    name = "{" + boxArr[findById(boxId)].parentIds[0] + "}-" + "{" + boxArr[findById(boxId)].parentIds[1] + "}";
...
function loadConfig() {
  var p1 = boxArr[findById(Number(parentId[0]))],
    p2 = boxArr[findById(parentId[1])];
  node1 = "Node #" + p1.id + " (" + p1.name + ")",
    node2 = "Node #" + p2.id + " (" + p2.name + ")";
  $("p").text(node1 + "-" + node2).attr("title", node1 + "-" + node2);
  changeVal();
  setConfig();
}
