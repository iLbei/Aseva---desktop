...
function changeVal() {
  var value = $('input').val();
  value = Number(value);
  var power = $('[name=power]').val() == "" ? $('[name=power]').attr('value') : $('[name=power]').val();
  var defaultV = $('[name=default]').val() == "" ? $('[name=default]').attr('value') : $('[name=default]').val();
  if (isNaN(Number(power))) power = $('[name=power]').attr('value');
  var name = "^" + power;
  if (!edit) {
    boxArr[boxArr.length - 1] = new Power(90, 30, name, "Power", selected_node_id, "orange", rank, 0, 0, [boxId], [], true, 'power', power, defaultV);
  } else {
    boxArr[findById(boxId)].power = power;
    boxArr[findById(boxId)].defaultV = defaultV;
...
function biOnInitEx(config, moduleConfigs) {
  ...
  if (edit) {
    ...
    var box = new Power(90, 30, "^0", "Power", selected_node_id, "orange", rank, 0, 0, [boxId], [], true, 'power', 0, 0);
    boxArr.push(box);
    setConfig();
  };
}
...
function loadConfig() {
  ...
  $('[name=power]').val(Boolean(arr.power) ? arr.power : 0);
  $('[name=default]').val(Boolean(arr.defaultV) ? arr.defaultV : 0);
}
