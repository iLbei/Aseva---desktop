...
function changeVal() {
  var text = $('.node1').text();
  var node1 = text.substring(text.indexOf("#") + 1, text.indexOf("("));
  node1 = node1.replace(/\s*/g, "");
  var name = "integral {" + node1 + "}";
  if (edit == "false") {
    if (swap)[parentId[0], parentId[1]] = [parentId[1], parentId[0]];
    boxArr[boxArr.length - 1] = new Box(90, 30, name, "Resettable integeral", selected_node_id, "orange", rank, 0, 0, parentId, [], true, 'reset_integral');
  } else {
    if (swap)[boxArr[findById(boxId)].parentIds[0], boxArr[findById(boxId)].parentIds[1]] = [boxArr[findById(boxId)].parentIds[1], boxArr[findById(boxId)].parentIds[0]];
    [boxArr[findById(boxId)].parentIds[0], boxArr[findById(boxId)].parentIds[1]] = [boxArr[findById(boxId)].parentIds[1], boxArr[findById(boxId)].parentIds[0]]
...
function biOnInitEx(config, moduleConfigs) {
  ...
  if (edit == "true") {
    ...
    var box = new Box(90, 30, "integral {" + parentId[0] + "}", "Resettable integeral", selected_node_id, "orange", rank, 0, 0, parentId, [], true, 'reset_integral');
    boxArr.push(box);
    setConfig();
  }
  if (biGetLanguage() == 1) {
