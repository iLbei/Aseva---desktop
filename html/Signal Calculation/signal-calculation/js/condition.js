...
function changeVal() {
  ...
  var choose = $('[name=reset]').get(0).checked;
  var box;
  var type = "Condition with number";
  var rank = boxArr[findById(boxId)].rank == "0" ? 1 : parseInt(boxArr[findById(boxId)].rank) + 1;
  // if (choose) {
  // if (num1 != "" && num2 != "") {
  var name = compareArr[first] + num1 + "(" + compareArr[second] + num2 + ")";
  if (edit == "false") {
    if (!flag) {
      box = new Condition(90, 30, name, type, selected_node_id, "orange", rank, 0, 0, [boxId], [], true, 'unary_condition', first, num1, second, num2, choose);
      boxArr.push(box);
    } else {
      boxArr[boxArr.length - 1] = new Condition(90, 30, name, type, selected_node_id, "orange", rank, 0, 0, [boxId], [], true, 'unary_condition', first, num1, second, num2, choose);
    }
  } else {
    boxArr[findById(boxId)].name = name;
    boxArr[findById(boxId)].first = first;
    boxArr[findById(boxId)].num1 = num1;
    boxArr[findById(boxId)].second = second;
    boxArr[findById(boxId)].num2 = num2;
    boxArr[findById(boxId)].flag = choose;
  }
  flag = true;
  setConfig();
}
...
function reset(obj) {
  if ($(obj).get(0).checked) {
    $(obj).parent().next().children('[name]').removeAttr('disabled').removeClass("disabled_background");
  } else {
    $(obj).parent().next().children('[name]').attr('disabled', true).addClass("disabled_background");
  }
}
...
function loadConfig() {
  ...
  reset($('[name=reset]'));
  $('[name=first]').val(Boolean(box.first) ? box.first : 0);
  $('[name=second]').val(Boolean(box.second) ? box.second : 0);
}
