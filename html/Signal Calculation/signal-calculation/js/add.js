...
function changeVal() {
  var value = $('input').val();
  value = Number(value);
  if (isNaN(value) || value == "") value = $('input').attr('value');
  var name = value >= 0 ? "+" + value : value;
  if (!edit) {
    boxArr[boxArr.length - 1] = new Add(90, 30, name, "Add / Subtract number", selected_node_id, "orange", rank, 0, 0, [boxId], [], true, 'add_number', value);
  } else {
...
function biOnInitEx(config, moduleConfigs) {
  ...
  if (edit) {
    ...
    var box = new Add(90, 30, "+0", "Add / Subtract number", selected_node_id, "orange", rank, 0, 0, [boxId], [], true, 'add_number', 0);
    boxArr.push(box);
    $('input').val(0);
    setConfig();
  };
...
function loadConfig() {
  ...
  $('input').val(Boolean(arr.value) ? arr.value : 0);
}
