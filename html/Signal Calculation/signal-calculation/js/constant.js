...
function changeVal() {
  var v = Number($('.constant').find('input').val());
  if (edit == "false") {
    boxArr[boxArr.length - 1] = new Constant(120, 30, v, "Input constant", selected_node_id, "blue", 0, 0, 0, [], [], true, 'constant', v);
  } else {
    boxArr[findById(boxId)].value = v;
    boxArr[findById(boxId)].name = v;
  }
  setConfig();
}
...
function biOnInitEx(config, moduleConfigs) {
  ...
  if (edit == "true") {
    ...
    var box = new Constant(120, 30, "0", "Input constant", selected_node_id, "blue", 0, 0, 0, [], [], true, 'constant', 0);
    boxArr.push(box);
    setConfig();
  };
}
...
function loadConfig() {
  ...
  $('[name=constantV]').val(Boolean(arr.value) ? arr.value : 0);
}
