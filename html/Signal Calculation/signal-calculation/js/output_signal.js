...
$('input[type=text]').on("input", function () {
  if (reg.test($(this).val())) {
    $(this).removeClass("red").addClass("springgreen");
  } else {
    $(this).removeClass("springgreen").addClass("red");
  }
  if (!$(this).hasClass("red")) changeVal();
})
...
function changeVal() {
  ...
  if (edit == "false") {
    boxArr[boxArr.length - 1] = new SignalOutput(120, 30, value, "Output signal", selected_node_id, "green", -1, 0, 0, [boxId], [], true, 'output', value);
  } else {
    boxArr[findById(boxId)].value = value;
    boxArr[findById(boxId)].name = value;
  }
  setConfig();
}
...
function biOnInitEx(config, moduleConfigs) {
  ...
  if (edit == "true") {
    ...
    var box = new SignalOutput(120, 30, "slot-0", "Output signal", selected_node_id, "green", -1, 0, 0, [boxId], [], true, 'output', "slot-0");
    boxArr.push(box);
    setConfig();
  };
}
...
function loadConfig() {
  var val = boxArr[findById(boxId)].value;
  $('[name=output_signal_name]').val(val).addClass("springgreen");
}
