...
function loadConfig(index, config) {
  ...
  $('.rsLidar32Le>.left table tr').each(function (i, v) {
    $(this).children('td:nth-of-type(2)').html(isNaN(toFix(pitch[i])) ? "0.00" : toFix(pitch[i]));
    $(this).children('td:nth-of-type(3)').html(isNaN(toFix(yaw[i])) ? "0.00" : toFix(yaw[i]));
  });
}
