...
$('[type=checkbox]').change(function () {
  ...
  if (name == "enabled") {
    check($(this));
  } else if (["RCTBGTSignalChecked", "RCTAGTSignalChecked"].includes(name)) {
...
function check(obj) {
  if ($(obj).is(":checked")) {
    ...
    clickRCTGtSignalChecked($('[name=RCTBGTSignalChecked]'));
    clickRCTGtSignalChecked($('[name=RCTAGTSignalChecked]'));
  } else {
    disAbled();
  }
}
...
//局部禁用启用-rctb/rcta left gt signal
...
function setConfig() {
  ...
  text += " />";
  biSetModuleConfig("rcta-rctb-evaluation.adasevaluation", text);
}
...
function loadConfig(obj) {
  $('[name]').each(function () {
    ...
    if (type == 'checkbox') {
      ...
      if(type=="text") $(this).attr("value",val)
      $(this).val(val);
    }
  });
  check($('[name=enabled]'));
