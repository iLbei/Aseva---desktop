//2023/11/16 v1.0.0 首个版本
var not_config = "";
$('[type=checkbox]').change(function () {
  var name = $(this).attr("name");
  if (name == "enabled") {
    check($(this));
  } else if (["raeb_gt_signal_checked"].includes(name)) {
    clickRAEBSignalChecked(this);
  }
});
...
$("a").click(function () {
  ...
  if (name == "message_id") {
    ...
    if (name == "raeb_target_ax_id") {
      var scale = $(this).attr("scale");
      biSelectSignal(name, $(this).attr('val'), false, null, true, scale, "[m/s²]");
    } else {
...
function check(obj) {
  if ($(obj).is(":checked")) {
    ...
    clickRAEBSignalChecked($('[name=raeb_gt_signal_checked]'));
  } else {
    disAbled();
  }
}
...
//全局禁用启用
function disAbled() {
  $('.container [name]:not(a,[name="enabled"])').each(function () {
    $(this).attr("disabled", "disabled").addClass("disabled_background");
  });
  $('.container,a').addClass('disabled_a');
...
function enAbled() {
  ...
  $('.container,a').removeClass("disabled_a");
  $(".tbody>table>tbody>tr>td:not(:nth-child(1))").attr("contenteditable", true);
}

//局部禁用启用-RAEB left gt signal
function clickRAEBSignalChecked(obj) {
  if ($(obj).is(":checked")) {
...
function setConfig() {
  ...
  $('[name]').each(function () {
    ...
    if (type == "checkbox") {
      ...
      if (key == "raeb_target_ax_id") {
        var scale = $(this).attr("scale");
        text += " raeb_target_ax_scale=\"" + scale + "\" ";
      }
      text += " " + key + "=\"" + txt + "\"";
    } else if (type == "number") {
      ...
      var v = $(this).val()
      if (v.trim() != "" && !(v.charAt(v.length - 1) == ",")) v += ",";
      text += " " + key + "=\"" + v + "\"";
    }
  });
  text += " raeb_vx_rel=\"" + ($("#raeb_vx_rel1").is(":checked") ? "yes" : "no") + "\"";
  text += " raeb_algorithm_config=\"";
  ...
  text += "\" />";
  biSetModuleConfig("raeb-evaluation.adasevaluation", text);
}

function biOnSelectedSignal(key, valueInfo, signBitInfo, scale) {
  if (valueInfo == null) {
    $("[name=" + key + "]").removeClass('green red').text(not_config).removeAttr("val title scale");
    ...
    if (key == "raeb_target_ax_id") $("[name=" + key + "]").attr("scale", scale);
  }
  setConfig();
}
...
function loadConfig(obj) {
    ...
    if (type == 'checkbox') {
      ...
      if (name == "message_id") {
        ...
        if (val != "" && val != "null") {
          ...
          if (name == "raeb_target_ax_id") {
            $(this).attr({ "scale": obj["raeb_target_ax_scale"] });
          }
        }
      }
    } else if (type == "radio") {
      if (val == "yes") {
        $("#raeb_vx_rel1").attr("checked", true);
      } else {
        $("#raeb_vx_rel2").attr("checked", true);
      }
    } else if (type == "text") {
      var v = val;
  ...
  var algorithm = obj["raeb_algorithm_config"].split(":");
