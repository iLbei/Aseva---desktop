...
// 2023/5/4: 插件6.5支持
// 2023/5/5: 补充注释
// 2023/5/10: 新增BIBusChannelType.SomeIP
// 2023/8/30: 插件6.8支持
...
* @param {Number} overwriteFreq 自定义脚本运行频率，若空则使用默认频率
* @param {Dictionary} overwriteParams 自定义参数，未设置的参数则使用默认值
*/
function BIPythonModuleConfig(enabled, moduleName, overwriteFreq, overwriteParams) {
  this.enabled = enabled;
  this.moduleName = moduleName;
  this.overwriteFreq = overwriteFreq;
...
/**
* 获取所有Python模板的ID及名称
* @returns {Dictionary} 返回字典对象，键为模板ID，值为模板名称
*/
function biGetPythonTemplateNames() {
  biUnimplemented("biGetPythonTemplateNames");
  return null;
}
...
/**
* 获取是否使用境内网络
* @returns {Boolean} 是否使用境内网络
*/
function biIsPRCWebPreferred() {
  biUnimplemented("biIsPRCWebPreferred");
  return false;
}
...
/**
* 获取Python模块配置(仅限模板模式)，获取完毕后会回调biOnQueriedPythonModuleConfig
* @param {String} templateID Python模板ID
* @param {String} idPostfix ID后缀
*/
function biQueryPythonModuleConfig(templateID, idPostfix) {
  biUnimplemented("biQueryPythonModuleConfig");
}

/**
* 获取Python模板信息，获取完毕后会回调biOnQueriedPythonTemplateInfo
* @param {String} templateID Python模板ID
*/
function biQueryPythonTemplateInfo(templateID) {
  biUnimplemented("biQueryPythonTemplateInfo");
}
...
* @param {String} templateID Python模板ID
* @param {String} idPostfix ID后缀
* @param {BIPythonModuleConfig} config Python模块配置
*/
function biSetPythonModuleConfig(templateID, idPostfix, config) {
  biUnimplemented("biSetPythonModuleConfig");
}
...
/**
* 在调用biQueryPythonModuleConfig返回结果后被调用
* @param {String} templateID Python模板ID
* @param {String} idPostfix ID后缀
* @param {BIPythonModuleConfig} config 模块配置信息，若模板不存在或ID后缀对应的模块不存在则为null
*/
function biOnQueriedPythonModuleConfig(templateID, idPostfix, config) {
}

/**
* 在调用biQueryPythonTemplateInfo返回结果后被调用
* @param {String} templateID Python模板ID
* @param {String} infoXmlBase64 Python模板XML的Base64编码表示，若模板不存在则为null
*/
