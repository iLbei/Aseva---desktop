...
function changeVal(obj) {
  ...
  dialogConfig[currentI]["typesLanes"]["color_sv"][index] = { "type": $(obj).attr("name"), "values": $(obj).val() };
  setConfig();
}
...
function loadConfig() {
  var colorArr = dialogConfig[currentI]["typesLanes"]["color_sv"];
  $('input').each(function () {
  ...
  for (var i in colorArr) {
    var v = colorArr[i]["values"] == "null" ? "" : colorArr[i]["values"];
    $("input[name=" + colorArr[i]["type"] + "]").val(v).attr("value", v);
  }
}