...
function loadConfig(config) {
  ...
  $('input[type=checkbox]').each(function () {
    ...
    val[name] == "yes" ? $(this).prop('checked', true) : $(this).removeAttr('checked');
  })
  var arr = val.arr;
  $('.table>div').html("")
  for (var i = 0; i < arr.length; i++) {
    var v = arr[i];
    var val = "<div class=\"fixclear input\"><div onclick=\"onClick(this)\" class=\"select\"></div><div><input type=\"text\" value=\"" + v.ip + "\" name=\"ip\" id=\"\"></div><div><input type=\"text\" value=\"" + v.user + "\" name=\"user\" id=\"\"></div><div><input type=\"text\" value=\"" + v.pass + "\" name=\"pass\" id=\"\"></div></div>";
...
function setConfig() {
  ...
  text += "</root>";
  biSetModuleConfig("gmsl-camera.gmslcamera", text);
}
